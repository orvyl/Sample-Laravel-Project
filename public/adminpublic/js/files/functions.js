$(function() {
    ...
    $('#modal_open1').click(function () {
        ...
        var numcheck = $('#frmdelmul input[type="checkbox"]:checked').length;
	...
	{
		$.ajax({
    		url: BaseUrl+"/admin/addnote",
    		type: "post",
    		data: $('#frmaddnote').serialize(),
    		success: function(data1){
    			data = $.parseJSON(data1);
    			c = $('.msgdiv');
    			c.html('<div class="nNote '+data.cl+'"><p>'+data.msg+'</p></div>');

    			if(data.cl == 'nSuccess')
    			{
    				$('#frmaddnote input').val('');
    				$('#frmaddnote textarea').val('');
    			}
	...
	{
		$('#frmdestroyprod').submit();
	}

	function fn_delete_product_mul()
	{
		$('#frmdelmul').submit();
	}
	...
	function fn_addOcc()
	{
		var newocc = $('#frmaddocc input[name="occname"]');
		var v = newocc.val();

		if(v != '')
		{
			$.ajax({
				url: BaseUrl + '/admin/occasions/addocc',
				data: { occname:v , occtype:newocc.attr('occt') },
    ...
    // Dialog Link
    $('#formDialog1_open').click(function () {
